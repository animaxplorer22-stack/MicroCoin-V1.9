#!/usr/bin/env python3
"""
MICROCORE (MCX) COMPLETE NODE v22.0 — MAINNET READY
...
def verify_signature(pub, msg, sig, miner_type):
    ...
    # ========== ESP32/PC/PHONE/PICO: ECDSA (128 chars) ==========
    try:
...
class MicroCoreNetwork:
    ...
    def _init_db(self):
        ...
        # Miners table with banned_until column
        c.execute('''CREATE TABLE IF NOT EXISTS miners (
            ...
            last_ping REAL, banned_until REAL)''')
    ...
    def _load(self):
        ...
        for row in self.conn.execute("SELECT * FROM miners"):
            self.miners[row[0]] = Miner(
                row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8] or 0,
                0, True, row[9], row[10], row[11], 0, row[12], row[13], row[14] or 0
            )
    ...
    def cleanup_inactive_miners(self):
        ...
        c.execute("SELECT vid, username FROM miners WHERE last_ping < ? AND active = 0", (timeout,))
        inactive = c.fetchall()
        
        for vid, username in inactive:
            c.execute("DELETE FROM miners WHERE vid=?", (vid,))
            if vid in self.miners:
                del self.miners[vid]
            logger.info(f"[CLEANUP] Removed inactive miner: {username}")
    ...
    def slash_miner(self, vid, reason, block_id=-1):
        ...
        self.conn.execute("UPDATE miners SET stake=?, level=?, slashes=?, active=?, banned_until=? WHERE vid=?",
                         (m.stake, m.level, m.slashes, m.active, m.banned_until, vid))
    ...
    def register_miner(self, vid, pub, username, wallet, stake, sig, timestamp, mtype="web"):
        ...
        self.conn.execute("INSERT OR REPLACE INTO miners VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                         (vid, pub, username, wallet, stake, level, 0, 0, 0, 0, 0, mtype, 0, 0, time.time(), 0, 0))
        self.conn.commit()
    ...
    def _register_self_miner(self):
        if self.username not in [m.username for m in self.miners.values()]:
            self.miners[self.username] = Miner(
                ...
                1000, 1, 0, 0, time.time(), True, 0, 0, 0, 0, "pc", 0, 0, 0
            )
            self.level_mgr.register(self.wallet, 1000)
            self.conn.execute("INSERT OR REPLACE INTO miners VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                             (self.username, self.pub, self.username, self.wallet,
                              1000, 1, 0, 0, 0, 0, 0, "pc", 0, 0, time.time(), 0, 0))
            self.conn.commit()
    ...
    def update_miner_uptime(self, vid, uptime_seconds, today_uptime):
        if vid in self.miners:
            ...
            self.miners[vid].active = True  # Reactivate on ping
            self.conn.execute("UPDATE miners SET uptime=?, today_uptime=?, last_ping=?, active=1 WHERE vid=?",
...
class MicroCoreServer:
    ...
    async def cleanup_loop(self):
        """Periodically cleanup inactive miners"""
        while not self._shutdown:
            try:
